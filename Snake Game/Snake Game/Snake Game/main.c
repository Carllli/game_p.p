...
#define LEFT 75 //왼쪽으로
#define RIGHT 77 //오른쪽으로
#define UP 72 //위로
#define DOWN 80 //아래로
#define PAUSE 112 //게임 일시정지
#define ESC 27 //게임 종료

#define MAP_X 3 //게임화면의 X위치
#define MAP_Y 2 //게임 화면의 Y위치
#define MAP_WIDTH 30 //게임 화면의 가로 길이
#define MAP_HEIGHT 20 //게임 화면의 세로 길이
...
void move(int dir) {
    ...
    if (dir == UP) --y[0];
    if (dir == DOWN) ++y[0];
    gotoxy(MAP_X + x[i], MAP_Y + y[i], "O"); //새로운 머리좌표값에 머리를 그림 
}
